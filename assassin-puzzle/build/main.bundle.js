...
var Vector = function () {
  function Vector(x, y, label) {
    var arrowheadSize = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 100;
...
var Ray = function Ray(center, direction) {
...
var Rectangle = function () {
  ...
  _createClass(Rectangle, [{
    ...
    value: function contains(vector) {
      return this.bottomLeftX <= vector.x && vector.x <= this.topRightX && this.bottomLeftY <= vector.y && vector.y <= this.topRightY;
    }
...
function createRectangleSVG(rectangle) {
  ...
  var rectangleSVG = svg.append("rect").attr("x", fromCartesianX(topLeft.x)).attr("y", fromCartesianY(topLeft.y)).attr("width", rectangle.width()).attr("height", rectangle.height());
  return rectangleSVG;
}

function rectangleStyle(rectangleSVG) {
  rectangleSVG.attr("stroke", "black").attr("stroke-width", 4).attr("fill", "none");
}

function createCircleSVG(point) {
  var circleSVG = svg.append('circle').attr("cx", fromCartesianX(point.x)).attr("cy", fromCartesianY(point.y)).attr("r", 6).attr("fill", point.labelToColor[point.label]).attr("stroke", point.labelToStrokeColor[point.label]).attr("stroke-width", 2);
  return circleSVG;
}

function createAssassinSVG(point) {
  var assassinPointSVG = createCircleSVG(point);
  // create a collection of lines representing the ray of the assassin's shot
}

function rayStyle(raySVG, id, color) {
  raySVG.attr("x1", function (d) {
    return fromCartesianX(assassin.x);
  }).attr("y1", function (d) {
    return fromCartesianY(assassin.y);
  }).attr("x2", function (d) {
    return fromCartesianX(d.x);
  }).attr("y2", function (d) {
    return fromCartesianY(d.y);
  }).attr("id", id).attr("stroke", color).attr("stroke-width", vectorStroke);
  return vectorSVG;
}
...
function randomPoint(square) {
  ...
  console.log("x in [" + minX + ", " + maxX + "], y in [" + minY + ", " + maxY + "]");
...
var assassinSVG = createCircleSVG(assassin);
